...
const RegisterPage = () => {
  ...
  return (
    ...
    <div className='flex justify-center items-center w-screen h-screen bg-[#262628] flex-col'>
      ...
      {
        currentStep ===4 ? 
        <button onClick={() => nav("/dashboard/home?query=Music")} style={{ background: 'linear-gradient(90deg, #E54B3C 0%, #FF1700 100%)', }} className='mt-3 rounded-md text-white text-sm h-[2.3rem] w-[20rem]'>Verify</button>:
      ...
      <button onClick={() => nav("/login")} className='mt-3 rounded-full text-white text-sm h-[2.3rem] w-[20rem]'>Sign In</button>
